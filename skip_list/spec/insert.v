...
Module AddSpec (Params: SKIP_LIST_PARAMS).
  ...
  Section Proofs.
    ...
    Theorem tryInsert_spec (curr head: node_rep) (key: Z) 
      ...
      INT_MIN < key < INT_MAX →
      {{{
        is_lazy_list lvlN head q Skeys bot from_bot_list
        ∗
      ...
      {{{ v new, RET v;
        is_lazy_list lvlN head q (Skeys ∪ {[ key ]}) bot from_bot_list
        ∗
        ( 
          ⌜ v = NONEV ⌝ ∨ 
          ( 
    ...
    Proof.
      ...
      + wp_lam. wp_pures.
        ...
        iIntros "_". wp_pures. 
        iModIntro. iApply "HΦ".
        iSplitR "Hown_tok Hown_auth_frag"; last by (iRight; iFrame).
        iExists (Sfrag ∪ {[ new ]}).
        iFrame "# ∗". iPureIntro.
    ...
    Theorem insert_spec (curr down head: node_rep) (key: Z) 
      ...
      INT_MIN < key < INT_MAX →
      {{{
        is_lazy_list lvlN head q Stop top (from_sub_list bot)
        ∗
      ...
      {{{ v new, RET v;
        is_lazy_list lvlN head q (Stop ∪ {[ key ]}) top (from_sub_list bot)
        ∗
        ⌜ v = SOMEV (rep_to_node new) ⌝ 
        ∗ 
        ⌜ node_key new = key ⌝
    ...
    Proof.
      iIntros (Hkey_range Φ) "(Hlazy & #Hown_curr & %Hrange & %Hdown & #Hown_down & Hown_tok) HΦ".
      iDestruct "Hlazy" as (Sfrag) "(%Hequiv & Hown_frag & #Hinv)".
        ...
        with "[Hown_auth Hown_succ Hown_tok Hlist]" as %Hneq.
      {
        ...
        iDestruct "HP" as (? ?) "(_ & _ & _ & Hown_tok' & %Hsucc)".
      ...
      clear Hperm Hsort Hequiv' S Skeys' L.

      iModIntro; wp_pures.
      set (new := (key, Some l', Some l'', lk)).
      ...
      iDestruct ("Hlist" with "[Hpt Hpt' Hpt'' Hlock Hown_tok]") as "Hlist".
      { 
        iNext; iFrame "# ∗". 
