...
From SkipList.arrays.spec Require Import insert.
...
Module AddSpec (Params: SKIP_LIST_PARAMS).
  ...
  Section Proofs.
    ...
    Theorem topLevel_spec (lvl h: nat) (key: Z) (top_head curr: node_rep) (S: gset Z) (q: frac)
    ...
    Proof.
      ...
      + inversion Hcase as [Heq]. 
        assert (h = lvl)%nat as <- by lia.
        iModIntro; iApply "HΦ". 
        iFrame "# ∗". 
      ...
      + assert (h ≠ lvl) by congruence.
        ...
        - iDestruct (own_valid_2 with "Hown_auth Hown_pred") 
          ...
          rewrite (list_equiv_invert_L lvl L top_head pred); last first.
    ...
    Theorem addAll_spec (lvl h: nat) (key: Z) (top_head curr: node_rep) (Skeys: gset Z) (q: frac)
      (bot: bot_gname) (top_sub: sub_gname) (bot_subs: list sub_gname) :
      INT_MIN < key < INT_MAX →
      {{{
        skip_list_equiv lvl top_head Skeys q bot (top_sub :: bot_subs)
        ∗
        (⌜ curr = top_head ⌝ ∨ own (s_auth top_sub) (◯ {[ curr ]}))
        ∗
        ⌜ node_key curr < key ⌝
      ...
      {{{ (v: val) (n: loc) (new: node_rep), RET v;
        skip_list_equiv lvl top_head (Skeys ∪ {[ key ]}) q bot (top_sub :: bot_subs)
        ∗
        ( 
          ⌜ v = NONEV ⌝ ∨ 
          ( 
            ...
            ∗
            (node_next new +ₗ lvl +ₗ 1) ↦∗{#1 / 2} replicate (h - lvl) #()
            ∗
            ∃ (γ: gname), 
    ...
    Proof.
      ...
      iRevert (Φ curr top_head lvl top_sub bot_subs) "Hlist Hown_curr Hrange Hh HΦ".
      iLöb as "IH".
      iIntros (Φ curr top_head lvl top_sub bot_subs) "Hlist #Hown_curr %Hrange %Hh HΦ".
      ...
      + iDestruct "Hlist" as "(%Hlvl & (Hown_frag & #Hinv))".
        ...
        iApply "HΦ". 
        rewrite loc_add_0.
        assert (h - 0 = h)%nat as -> by lia.

        assert (1 / 2 / 2 = lfrac 0%nat)%Qp as ->.
        {
          rewrite /lfrac.
          assert (0 + 2 = 2)%nat as -> by lia.
          rewrite /= Qp_mul_1_r -Qp_div_div //.
        }
        ...
        - wp_bind (BinOp _ _ _).
          ...
          * wp_op.
            ...
            ++ rewrite Hopt. wp_pures.
               ...
               {
                 rewrite /lfrac.
                 assert (lvl - 1 + 2 = S lvl)%nat as -> by lia.
                 assert (lvl + 2 = S (S lvl))%nat as -> by lia.
                 rewrite /= Qp_div_div Qp_mul_comm //.
               }

               rewrite loc_add_assoc.
               assert ((lvl - 1)%nat + 1 = lvl) as -> by lia.
               assert (h - (lvl - 1) = S (h - lvl))%nat as -> by lia.
               rewrite replicate_S array_cons.
               iDestruct "Hnext" as "(Hnext & Hreplicate)".

               iDestruct "Hlock" as (γ) "(#Hlock & Hin_lock & Hlocked)".
               iDestruct "Hin_lock" as (vs) "(Hnext' & %Hlength)".
               pose proof (list_split vs h lvl) as Hsplit.
               ...
               iDestruct "Hnext'" as "(Hnext'1 & Hnext' & Hnext'2)".
               ...
               iCombine "Hnext'1 Hnext Hnext'2" as "Hnext".
    ...
    Theorem add_spec (key: Z) (height: nat) (v: val) (S: gset Z) (q: frac)
      (bot: bot_gname) (subs: list sub_gname)
      (Hrange: INT_MIN < key < INT_MAX) 
      (Hheight: height ≤ MAX_HEIGHT) :
