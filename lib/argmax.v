...
Inductive argmax A `{Countable A} :=
| ArgMax : gset A → Z → argmax A.
Global Arguments ArgMax {_ _ _} _.
...
Section cmra.
  ...
  Canonical Structure argmaxO := leibnizO (argmax A).
  ...
  Local Instance argmax_op_instance : Op (argmax A) := λ n m,
    match m, n with
    | ArgMax a i, ArgMax b j => if (decide (i = j)) then ArgMax (a ∪ b) i
