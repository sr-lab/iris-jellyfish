...
Section lemmas.
  ...
  Proof.
    iIntros (?) "Hwp %Φ %x Hα Hαβ HΦ".
    iApply (wp_step_fupd _ _ ⊤ _ (∀.. y, β x y -∗ ∀.. z, POST x y z -∗? Φ (f x y z))
      with "[$HΦ //]"); first done.
    iApply (atomic_wp_seq with "Hwp Hα Hαβ []").
    iIntros (y) "Hβ %z Hpost HΦ". iModIntro.
    iApply ("HΦ" with "Hβ Hpost").
  Qed.
  ...
  Proof.
    intros ?. iIntros "Hwp #Hinv" (Φ) "AU".
    iApply "Hwp". iApply ainv_aupd. iAuIntro.
    ...
    - (* commit *)
      iIntros (y). rewrite ->!tele_app_bind.
      iIntros "[HI Hβ]". iModIntro. iRight.
      iExists y.
      ...
      * iIntros "[HI $]". iModIntro. iIntros "AU". iModIntro. iFrame.
        iApply ares_aupd. iAuIntro. iApply (aacc_aupd with "AU"); first solve_ndisj.
        clear z. iIntros (z) "Hα". unfold atomic_acc. iModIntro.
        iExists z. iFrame. iSplit.
        + do 2 (iIntros; iModIntro; iFrame).
        + iIntros (?) "Hα". iModIntro. rewrite ->!tele_app_bind. iLeft. iFrame.
          iIntros "AU". by iMod (
            ares_commit _ _ _ (λ x y, ∀.. z, POST x y z -∗? Φ (f x y z))%I
          with "AU").
